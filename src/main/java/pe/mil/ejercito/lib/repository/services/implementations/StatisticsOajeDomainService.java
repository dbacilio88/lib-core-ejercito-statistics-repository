...
public class StatisticsOajeDomainService extends ReactorServiceBase implements IStatisticsOajeDomainService {
    ...
    @Override
    public Mono<StatisticsOajeDto> getByIdEntity(Long id) {
        ...
        final Optional<EpStatisticsOajeEntity> persistenceEntity = this.repository.findById(id);

        return getStatisticsOajeDto(
            persistenceEntity,
    ...
    @Override
    public Mono<StatisticsOajeDto> getByUuIdEntity(String uuId) {
        ...
        final Optional<EpStatisticsOajeEntity> persistenceEntity = this.repository.findByUuId(uuId);
    ...
    @Override
    public Mono<StatisticsOajeDto> updateEntity(StatisticsOajeDto dto) {
    ...
    private Mono<StatisticsOajeDto> doOnUpdate(StatisticsOajeDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                ...
                final Optional<EpStatisticsOajeEntity> statisticsOajeEntity = this.repository.findByUuId(request.getUuId());
                if (statisticsOajeEntity.isEmpty()) {
                ...
                final EpStatisticsOajeEntity entityUpdate = statisticsOajeEntity.get();
                entityUpdate.setStDocument(documentRegisterEntity.get());
                entityUpdate.setStUpdatedDate(Instant.now());
                final EpStatisticsOajeEntity entityResult = this.repository.save(entityUpdate);
                entityUpdate.setStDocument(documentRegisterEntity.get());
                return Mono.just(this.mapper.mapperToDto(entityResult));
    ...
    @Override
    public Mono<StatisticsOajeDto> deleteByUuIdEntity(String uuId) {
        if (Boolean.TRUE.equals(CommonRequestHelper.isInvalidUuId(uuId))) {
    ...
    private Mono<StatisticsOajeDto> getStatisticsOajeDto(Optional<EpStatisticsOajeEntity> brEntity, String successMessage, String messageExist) {
    ...
    @Override
    public Mono<List<StatisticsOajeDto>> getAllEntities(String document, String limit, String page, PageableDto pageable) {
        Pageable paging = PageRequest.of(Integer.parseInt(page) - 1, Integer.parseInt(limit));
        Page<EpStatisticsOajeEntity> entityPage = this.repository.findAll(document, paging);
        List<StatisticsOajeDto> brigades = this.mapper.mapperToList(entityPage.getContent());
