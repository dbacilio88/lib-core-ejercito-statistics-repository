...
import pe.mil.ejercito.lib.repository.components.mappers.IStatisticsDieMapper;
import pe.mil.ejercito.lib.repository.components.validations.IStatisticsDieValidation;
...
import pe.mil.ejercito.lib.repository.repositories.IEpStatisticsDieRepository;
...
import pe.mil.ejercito.lib.repository.services.contracts.IStatisticsDieDomainService;
...
/**
 * StatisticsDieDomainService
 * <p>
 * StatisticsDieDomainService class.
 * <p>
...
@Log4j2
@Service
public class StatisticsDieDomainService extends ReactorServiceBase implements IStatisticsDieDomainService {

    private final IEpStatisticsDieRepository repository;
    private final IEpDocumentRegisterRepository documentRegisterRepository;
    private final IStatisticsDieMapper mapper;

    public StatisticsDieDomainService(final IEpStatisticsDieRepository repository,
                                      final IEpDocumentRegisterRepository documentRegisterRepository,
                                      final IStatisticsDieMapper mapper) {
    ...
    @Override
    public Mono<StatisticsDieDto> getByIdEntity(Long id) {
        ...
        final Optional<EpStatisticsDieEntity> persistenceEntity = this.repository.findById(id);

        return getStatisticsDieDto(
            persistenceEntity,
    ...
    @Override
    public Mono<StatisticsDieDto> getByUuIdEntity(String uuId) {
        ...
        final Optional<EpStatisticsDieEntity> persistenceEntity = this.repository.findByUuId(uuId);
    ...
    @Override
    public Mono<StatisticsDieDto> saveEntity(StatisticsDieDto dto) {
        return doOnSave(dto)
    ...
    @Override
    public Mono<StatisticsDieDto> updateEntity(StatisticsDieDto dto) {
    ...
    private Mono<StatisticsDieDto> doOnValidateResponse(StatisticsDieDto dto) {
        return IStatisticsDieValidation.doOnValidationResponse().apply(dto)
            .flatMap(validation -> {
    ...
    private Mono<StatisticsDieDto> doOnSave(StatisticsDieDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                final EpStatisticsDieEntity persistenceEntity = this.mapper.mapperToEntity(request);
    ...
    private Mono<StatisticsDieDto> doOnUpdate(StatisticsDieDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                ...
                final Optional<EpStatisticsDieEntity> statisticsDieEntity = this.repository.findByUuId(request.getUuId());
                if (statisticsDieEntity.isEmpty()) {
                ...
                final EpStatisticsDieEntity entityUpdate = statisticsDieEntity.get();
    ...
    @Override
    public Mono<StatisticsDieDto> deleteByUuIdEntity(String uuId) {
        if (Boolean.TRUE.equals(CommonRequestHelper.isInvalidUuId(uuId))) {
    ...
    private Mono<StatisticsDieDto> getStatisticsDieDto(Optional<EpStatisticsDieEntity> brEntity, String successMessage, String messageExist) {
    ...
    @Override
    public Mono<List<StatisticsDieDto>> getAllEntities(String document, String limit, String page, PageableDto pageable) {
        Pageable paging = PageRequest.of(Integer.parseInt(page) - 1, Integer.parseInt(limit));
        Page<EpStatisticsDieEntity> entityPage = this.repository.findAll(document, paging);
        List<StatisticsDieDto> brigades = this.mapper.mapperToList(entityPage.getContent());
