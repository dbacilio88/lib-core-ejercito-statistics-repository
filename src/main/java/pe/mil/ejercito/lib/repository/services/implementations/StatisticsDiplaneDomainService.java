    ...
    private Mono<StatisticsDiplaneDto> doOnUpdate(StatisticsDiplaneDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                ...
                final Optional<EpStatisticsDiplaneEntity> statisticsAeEntity = this.repository.findByUuId(request.getUuId());
                ...
                final EpStatisticsDiplaneEntity entityUpdate = statisticsAeEntity.get();
