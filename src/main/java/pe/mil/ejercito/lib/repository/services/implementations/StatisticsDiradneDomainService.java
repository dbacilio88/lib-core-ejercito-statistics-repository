    ...
    private Mono<StatisticsDiradneDto> doOnUpdate(StatisticsDiradneDto dto) {
        return Mono.just(dto)
            .flatMap(request -> {
                ...
                final Optional<EpStatisticsDiradneEntity> statisticsDiradneEntity = this.repository.findByUuId(request.getUuId());
                if (statisticsDiradneEntity.isEmpty()) {
                ...
                final EpStatisticsDiradneEntity entityUpdate = statisticsDiradneEntity.get();
